...
Current RC architecture gaps are tracked in `docs/RC_COMPLETION_MATRIX.md`. The items below are
product/environment limitations retained from the 0.3.0 terminal baseline.
...
- Typed Task Graph execution/restart recovery and the isolated Skill Workspace are not complete.
- Primitive Tool coverage is incomplete; existing mining/smelting/storage/crafting/defense
  capabilities are still transitional composite wrappers rather than proven primitive-only Skills.
- No Live external Brain verification or human playtest has been claimed.
