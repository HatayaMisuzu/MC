...
| Mine | PARTIAL | `resource.mine_vein` dispatches bounded `MineResourceVein`; discovers a connected six-neighbor vein, accumulates vanilla hardness progress, breaks through `ServerPlayerGameMode`, and collects actual drops before accepting verified inventory deltas | Runtime schema test; real Fabric GameTest mines two connected diamond ores, verifies both world changes, two vanilla drops, iron-pickaxe durability >=2, and audited action-path evidence | navigation to distant veins, mixed/common resources, tool break/full inventory/world change/restart breadth, Brain E2E |
...
| MCP / Tool Protocol | PARTIAL | authenticated `/mcp` Streamable HTTP JSON-RPC endpoint negotiates MCP `2025-03-26`/`2025-06-18`, exposes only context-filtered MCAC Tool Gateway definitions, binds calls/cancellation to controller + Brain session + companion, and returns structured terminal observations | Runtime HTTP integration verifies auth, negotiation, required binding headers, bounded tool list, rejected missing companion, stable call identity and cancellation notification | SSE progress/event resumability, protocol-version header enforcement, concurrent cancellation E2E, Hermes native MCP configuration and protocol doctor |
