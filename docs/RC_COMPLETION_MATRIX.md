...
| Async Tool Loop | LOCALLY_VERIFIED | durable session/call/task/behavior binding; immediate PAUSED/RECONCILIATION terminal mapping; timeout cancellation confirmation | Runtime Tool Gateway tests; Hermes Replay + prior real Fabric E2E | Remote verification of current SHA; concurrent cancel without companion-lock delay |
...
| External ASK_USER | PARTIAL | structured Hermes/OpenAI-compatible question; durable brainSession/task binding; Web/game routing; idempotent answer | adapter, repository restart, coordinator resume, HTTP/game Replay tests | full 6/16 withdraw-return-deliver E2E and offline reconnect delivery |
