...
| Search | PARTIAL | bounded query/open/citations/cancel gateway and privacy filtering; Terminal/Web configuration covers explicit enable/disable, HTTPS-or-loopback endpoint, environment-only token, 1..30s timeout, and non-overlapping allow/deny domain policy; pairing writes validated policy into `runtime.yml`; general Doctor checks safe-disabled/configured token-source readiness and the Search page runs an explicit bounded live provider probe without exposing credentials or response bodies | Runtime Replay tests; Search configuration/YAML propagation and live-probe contract tests; Search page Vitest; production Web build | cache, source-click UI, full search-session lifecycle and remote/live provider verification |
...
| Reliability | PARTIAL | WAL, leases, restart interruption, dedupe; Runtime/Fabric inspection polling tolerates an isolated transport timeout while retaining bounded 10..20s scenario deadlines and final-error evidence | restart/persistence/E2E tests; local real Fabric E2E after polling hardening; remote heavy workflow `29476034674` passed Runtime/Fabric E2E, restart, and multi-profile validation | long-run, reconnect/backoff, leak/performance breadth |
