...
Status: Closed historical execution tracker
Updated: 2026-07-29
...
| 8 | Complete clean-candidate local gates and exact release validation | `COMPLETED` | Frozen implementation candidate `8aa284f3f44c8728658232a3cd01d75acd4ec900` passes clean shared checks and all Loader builds/launches; Fabric 31/31 with 30 created/30 removed, Forge 4/4 and NeoForge 1/1 GameTests; both Runtime E2Es; both two-process restart gates; multi-Profile and Runtime-disabled launch; Brain reconnect, 100-turn and soak; package, arbitrary-directory HTML start, clean-extraction Golden Path and warning-all check; Web exact install, zero-vulnerability production audit, lint, 15/15 unit tests, build and real-backend E2E. The one externally interrupted Runtime/Fabric process is preserved as `EXTERNAL_ENVIRONMENT_FAILURE`; its single permitted recovery run passed. The SHA-bound ignored ledger records every command, exit, log and source-change state. |
| 9 | PR, exact candidate CI, ordinary merge, frozen-main package, immutable tag and Release | `COMPLETED` | Final repair candidate `5235fbb18f35c5ad7fff8ef207ff02a540588db9` was merged through PR #26 with ordinary merge commit `747c7e8046073d9534eae6ae775645341be4cdcd`. PR Fast `30436629059` and Supply chain/CodeQL `30436629064` passed; earlier Heavy `30433326289` passed. Per the user's later override, the running Windows workflow was not awaited and no complete suite was rerun. The exact-source package was published under annotated tag and GitHub Release `mcac-productization-baseline-0.3.1`; the immutable 0.3.0 tag was not changed. |
| 10 | Final full-audit report and item-by-item acceptance | `COMPLETED` | `docs/product/MCAC_0.3.1_FULL_AUDIT_AND_REPAIR_REPORT.md` records actual candidate, merge, tag, Release, asset hashes and the honest remote-run boundary. Live Hermes and human play remain pending. |
...
The next exact-candidate GameTest run stopped at the Fabric mining acceptance rather than being
rerun. The registered mock Owner and Companion initially occupied the same position, so either
real `ServerPlayer` could collect the two real diamond `ItemEntity` drops while the behavior
correctly waited for the Companion's declared inventory delta. The fixture now uses a dedicated
forced arena, moves Owner eight blocks outside pickup range, and removes the former post-registration
UUID mutation. `gameTestFixtureCheck` rejects either boundary regressing. The focused real-server
validation passes 31/31 with exactly 30 Companions created and 30 removed, no timeout, and no Owner
leak.
