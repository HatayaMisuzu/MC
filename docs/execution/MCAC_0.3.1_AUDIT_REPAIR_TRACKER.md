...
| 9 | PR, exact candidate CI, ordinary merge, exact-main rebuild/CI, immutable tag and Release | `IN_PROGRESS` | Target version/tag are `0.3.1` / `mcac-productization-baseline-0.3.1`; the 0.3.0 tag still resolves to `ec56aca171f4bc7ffb43eca09cad07678b225163` and must remain unchanged. Closeout commit and exact-SHA remote workflows are next. |
| 10 | Final full-audit report and item-by-item acceptance | `IN_PROGRESS` | `docs/product/MCAC_0.3.1_FULL_AUDIT_AND_REPAIR_REPORT.md` records the audit population, repairs, frozen-candidate local gates, candidate hashes, retained limitations and pending Phase 9 identities. It becomes final after remote/merge/tag/Release outcomes are known. |
