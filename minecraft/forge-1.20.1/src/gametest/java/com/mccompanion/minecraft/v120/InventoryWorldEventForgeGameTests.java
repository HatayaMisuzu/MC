...
/** Real 1.20.1 inventory and exact task-target observation evidence. */
@GameTestHolder(MinecraftAiCompanionForge.MOD_ID)
@PrefixGameTestTemplate(false)
public final class InventoryWorldEventForgeGameTests {
    private InventoryWorldEventForgeGameTests() { }

    @GameTest(batch = "inventoryWorldEvents", templateNamespace = "minecraft",
            template = "bastion/mobs/empty", timeoutTicks = 100)
    public static void inventoryThresholdGoalAndTargetEdgesUseRealState(GameTestHelper helper) {
        FakeConnection connection = new FakeConnection();
        ServerPlayer owner = new ServerPlayer(helper.getLevel().getServer(), helper.getLevel(),
                new GameProfile(UUID.randomUUID(), "forge-inventory-world-owner"));
        owner.moveTo(helper.absolutePos(new BlockPos(1, 1, 1)), 0.0F, 0.0F);
        ...
        connection.disconnect(Component.literal("Forge inventory/world event GameTest complete"));
        helper.succeed();
    }
