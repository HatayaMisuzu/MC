...
public final class ThreatRecoveryForgeGameTests {
    @GameTest(batch = "threat_recovery", templateNamespace = "minecraft", template = "bastion/mobs/empty", timeoutTicks = 1600)
    public static void lowHealthRetreatEatsAndResumesDurableBlueprint(GameTestHelper helper) {
    ...
    @GameTest(batch = "threat_recovery", templateNamespace = "minecraft", template = "bastion/mobs/empty", timeoutTicks = 1200000)
    public static void runtimeBridgeRecoversDurableGraph(GameTestHelper helper) {
        if (!Boolean.getBoolean("mccompanion.threat.e2e")) { helper.succeed(); return; }
        runLowHealthRecovery(helper);
    }
    ...
    @GameTest(batch = "threat_recovery", templateNamespace = "minecraft", template = "bastion/mobs/empty", timeoutTicks = 300)
    public static void creeperPreemptsMultipleEnemiesAndCancellationStopsRecovery(GameTestHelper helper) {
    ...
    @GameTest(batch = "threat_recovery", templateNamespace = "minecraft", template = "bastion/mobs/empty", timeoutTicks = 400)
    public static void skeletonThreatUsesLargerSafeDistance(GameTestHelper helper) {
    ...
    @GameTest(batch = "threat_recovery", templateNamespace = "minecraft", template = "bastion/mobs/empty", timeoutTicks = 500)
    public static void ownerDamageInvokesBoundedCombatThenReturnsToRoute(GameTestHelper helper) {
    ...
    @GameTest(batch = "threat_recovery", templateNamespace = "minecraft", template = "bastion/mobs/empty", timeoutTicks = 200)
    public static void missingFoodFailsAndPauseCancelsEatingWithoutAutomaticResume(GameTestHelper helper) {
    ...
    @GameTest(batch = "threat_recovery", templateNamespace = "minecraft", template = "bastion/mobs/empty", timeoutTicks = 600)
    public static void explicitOwnerDefenseUsesSharedContinuousCombat(GameTestHelper helper) {
    ...
    private static void startBlueprint(CombatForgeGameTests.Fixture f, SmallBlueprint plan, Runnable action) {
        if (Boolean.getBoolean("mccompanion.threat.e2e")) {
    ...
    private static void awaitExternalBlueprint(CombatForgeGameTests.Fixture f, long deadline, Runnable action) {
    ...
    private static void finishPublished(CombatForgeGameTests.Fixture f, Mob target, long deadline) {
        if (!Boolean.getBoolean("mccompanion.threat.e2e")
    ...
    private static void ready(CombatForgeGameTests.Fixture f, int ticks, java.util.function.Consumer<CombatForgeGameTests.Fixture> action) {
    ...
    private static Map<String, String> details(CombatForgeGameTests.Fixture f) {
        var observation = f.snapshot().behaviorObservation();
        return observation == null ? Map.of() : observation.details();
    }

    private static String code(CombatForgeGameTests.Fixture f) {
        var observation = f.snapshot().behaviorObservation();
    ...
    private static CompanionSavedData data(CombatForgeGameTests.Fixture f) {
    ...
    private static void moveAway(CombatForgeGameTests.Fixture f, Mob mob, int ticks) {
