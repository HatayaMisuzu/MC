...
/** Read-only bounded observations of the actual Forge connected body and nearby world. */
public final class PrimitiveObservationService {
    ...
    private static Result block(CompanionPlayer body, JsonNode position) {
        ...
        String dimension = position.path("dimension")
                .asText(body.serverLevel().dimension().location().toString());
        ...
        var hit = body.serverLevel().clip(new ClipContext(
                body.getEyePosition(),
    ...
    private static Result item(CompanionPlayer body, String rawItem) {
        ...
        for (int slot = 0; slot < body.getInventory().getContainerSize(); slot++) {
            ...
            slots.add(JSON.createObjectNode()
                    .put("slot", slot)
        ...
        observation.put("totalCount", total)
    ...
    private static Result entities(CompanionPlayer body, JsonNode arguments) {
        ...
        if (!Double.isFinite(radius) || radius < 1.0D || radius > 16.0D) {
            return failure("OBSERVATION_RADIUS_INVALID",
                    "Entity radius must be between 1 and 16 blocks");
        }
        if (limit < 1 || limit > 32) {
            return failure("OBSERVATION_LIMIT_INVALID",
                    "Entity result limit must be between 1 and 32");
        }
        if (!type.isBlank()) {
            ResourceLocation typeId = ResourceLocation.tryParse(type);
            if (typeId == null || !BuiltInRegistries.ENTITY_TYPE.containsKey(typeId)) {
                return failure("ENTITY_TYPE_NOT_FOUND",
                        "Entity type is not present in the live Registry");
            }
        }
        UUID exact = entityId.isBlank() ? null : UUID.fromString(entityId);
        var matches = body.serverLevel()
                .getEntities(body, body.getBoundingBox().inflate(radius), entity -> {
                ...
                .toList();
        ObjectNode observation = envelope(body, "ENTITY")
                .put("radius", radius)
    ...
    private static ObjectNode envelope(CompanionPlayer body, String kind) {
        return JSON.createObjectNode()
                .put("source", "LIVE_SERVER_OBSERVATION")
                .put("kind", kind)
                .put("verified", true)
