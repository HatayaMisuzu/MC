...
/** Bounded, read-only Forge 1.20.1 views over live registries and recipes. */
public final class RegistryObservationService {
    ...
    public static Result registry(MinecraftServer server, JsonNode arguments) {
        try {
            ...
            if (arguments.path("tool").asText("").equals("registry.describe")) {
                return describe(server, kind, arguments.path("id").asText(""));
            }
            return search(server, kind, arguments.path("query").asText(""),
    ...
    public static Result recipes(MinecraftServer server, JsonNode arguments) {
        try {
            ...
            if (!type.equals("SMELTING")) {
                for (CraftingRecipe recipe : server.getRecipeManager().getAllRecipesFor(RecipeType.CRAFTING)) {
                    ObjectNode value = recipe(server, recipe, "CRAFTING");
            ...
            if (!type.equals("CRAFTING")) {
                for (SmeltingRecipe recipe : server.getRecipeManager().getAllRecipesFor(RecipeType.SMELTING)) {
                    ObjectNode value = recipe(server, recipe, "SMELTING");
                    if (matches(value, query, output)) matches.add(value);
                }
            }
    ...
    private static Result search(
        ...
        List<ResourceLocation> matches = keys(server, kind)
                ...
                .filter(value -> normalized.isBlank()
                        || value.toString().toLowerCase(Locale.ROOT).contains(normalized))
        ...
        ObjectNode observation = envelope("LIVE_SERVER_REGISTRY")
                ...
                .put("truncated", matches.size() > limit)
                .put("limit", limit);
        if (!normalized.isBlank()) observation.put("query", normalized);
    ...
    private static Result describe(MinecraftServer server, String kind, String rawId) {
        ...
        if (keys(server, kind).noneMatch(id::equals)) {
            return new Result(false, "REGISTRY_ID_NOT_FOUND",
                    envelope("LIVE_SERVER_REGISTRY")
                            .put("kind", kind)
                            .put("id", rawId)
                            .put("present", false));
        }
        ...
        ObjectNode details = observation.with("entry").with("details");
        switch (kind) {
            case "ITEM" -> {
            ...
            case "BLOCK" -> {
                ...
                details.put("hasItemForm", itemForm != null
                        && !itemForm.toString().equals("minecraft:air"));
    ...
    private static ObjectNode recipe(MinecraftServer server, Recipe<?> recipe, String type) {
        ...
        recipe.getIngredients().stream().limit(16).forEach(valueIngredient ->
                ingredients.add(ingredient(valueIngredient)));
        value.put("ingredientCount", recipe.getIngredients().size())
                .put("ingredientsTruncated", recipe.getIngredients().size() > 16);
        if (recipe instanceof CraftingRecipe crafting) {
            ...
            value.put("cookingTime", smelting.getCookingTime())
                    .put("experience", smelting.getExperience())
    ...
    private static boolean matches(ObjectNode recipe, String query, String output) {
        ...
        return query.isBlank()
                || recipe.path("id").asText().toLowerCase(Locale.ROOT).contains(query)
    ...
    private static ObjectNode envelope(String source) {
        return JSON.createObjectNode()
                ...
                .put("trust", "VERIFIED_LIVE_SERVER")
                .put("confidence", "HIGH");
    }

    private static void boundedTags(
            ObjectNode parent,
            Stream<? extends net.minecraft.tags.TagKey<?>> tags) {
        ArrayNode target = parent.putArray("tags");
        List<String> values = tags.map(tag -> tag.location().toString())
                .sorted()
                .limit(65)
                .toList();
