...
public final class CompanionCommands {
    ...
    private static int playerText(CommandContext<CommandSourceStack> context, PlayerTextGateway gateway)
        ...
        if (text.isEmpty() || text.length() > 512) {
            context.getSource().sendFailure(Component.literal("请输入 1..512 个字符的 Companion 请求。"));
            return 0;
        }
    ...
    @FunctionalInterface
    public interface PlayerTextGateway {
        TextRequestResult submit(ServerPlayer owner, String text);
    }
