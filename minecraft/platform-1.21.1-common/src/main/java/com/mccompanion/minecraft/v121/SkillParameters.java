...
public record SkillParameters(String capability, String itemId, int quantity, boolean allowPartial,
                              ...
                              String targetReferenceKind, String targetName, Integer targetRuntimeId,
                              Double minimumDistance, Double maximumDistance, Integer lostTimeoutTicks,
    ...
    public SkillParameters {
        ...
        if (targetRuntimeId != null && targetRuntimeId < 0) throw new IllegalArgumentException("targetRuntimeId must be non-negative");
        if (minimumDistance != null && (!Double.isFinite(minimumDistance) || minimumDistance < 0.0D || minimumDistance > 64.0D)) {
            throw new IllegalArgumentException("minimumDistance must be 0..64");
        }
        if (maximumDistance != null && (!Double.isFinite(maximumDistance) || maximumDistance < 0.0D || maximumDistance > 64.0D)) {
