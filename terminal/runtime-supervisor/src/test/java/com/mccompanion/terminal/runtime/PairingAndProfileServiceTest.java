...
class PairingAndProfileServiceTest{
 ...
 @Test void occupiedPortIsSkippedForNewProfile()throws Exception{
  try(ServerSocket occupied=new ServerSocket()){
   occupied.bind(new InetSocketAddress(InetAddress.getLoopbackAddress(),8766));
   var profile=new RuntimeProfileService(temp.resolve("home"),temp.resolve("runtime.exe")).ensure("a");
   assertEquals(8767,profile.port());
  }
 }

 @Test void concurrentAllocatorsNeverChooseSamePort()throws Exception{
  Path home=temp.resolve("home");
  ExecutorService pool=Executors.newFixedThreadPool(2);
  try{
   CountDownLatch ready=new CountDownLatch(2),go=new CountDownLatch(1);
   Callable<RuntimeProfile> first=()->{ready.countDown();go.await();return new RuntimeProfileService(home,temp.resolve("runtime.exe")).ensure("a");};
   Callable<RuntimeProfile> second=()->{ready.countDown();go.await();return new RuntimeProfileService(home,temp.resolve("runtime.exe")).ensure("b");};
